...
import {
  POS, PASS, COMPARE_YEARS, semOrder, batchPO, batchCourses,
  courseAttainment, studentPO, avgOf, visibleBatches,
} from '../data/index.js';
...
export default function LecturerBatchesView() {
  ...
  if (lect.compare && b.history) {
    series.push({ values: b.history[lect.compareYear] || POS.map(() => 0), color: 'var(--ink-3)', stroke: 'var(--ink-3)', muted: true });
  }
  ...
  const pickBatch = (id) => dispatch({ type: 'SET_LECT', patch: { batch: id, compare: false, compareYear: 2025, sem: 1 } });

  const cmpYear = (d) => {
    const y = Math.max(COMPARE_YEARS[COMPARE_YEARS.length - 1], Math.min(COMPARE_YEARS[0], lect.compareYear + d));
  ...
  const doExport = () => {
    ...
    b.students.forEach(id => {
      const s = students[id];
      Object.keys(s.courseScores).forEach(code => rows.push([s.matric, s.name, code, s.courseScores[code]]));
    });
  ...
  const allBatchesPO = POS.map((_, p) =>
    avgOf(Object.keys(batches).map(id => batchPO(id, 'all', batches, students, courses)[p]).filter(v => v > 0))
  );
          ...
          <main>
            <div className="page-head" style={{ marginBottom: 14 }}>
              ...
              <div style={{ display: 'flex', gap: 10, alignItems: 'center', flexWrap: 'wrap' }}>
                <label className="tog" style={{ fontSize: 13 }}>
                  <input type="checkbox" checked={lect.compare} onChange={() => dispatch({ type: 'SET_LECT', patch: { compare: !lect.compare } })} />
                  Compare by year
                </label>
                {lect.compare && (
                  <div className="stepper" role="group" aria-label="Comparison year">
                    <button onClick={() => cmpYear(-1)} disabled={lect.compareYear <= COMPARE_YEARS[COMPARE_YEARS.length - 1]}>‹</button>
                    <span className="val mono">{b.name} · {lect.compareYear}</span>
                    <button onClick={() => cmpYear(1)} disabled={lect.compareYear >= COMPARE_YEARS[0]}>›</button>
                  </div>
                )}
            ...
            <div className="card">
              ...
              <div className="legend">
                ...
                {lect.compare && <span><span className="swatch" style={{ background: 'var(--line-2)' }} />{b.name} · {lect.compareYear}</span>}
            ...
            <div className="card" style={{ marginTop: 16 }}>
              ...
              <table>
                <thead><tr><th>Sem</th><th>Code</th><th>Course</th><th>Contributes to</th><th style={{ textAlign: 'right' }}>Attainment</th><th>Status</th></tr></thead>
                <tbody>
                  {coursesInBatch.length ? coursesInBatch.map(c => {
                    ...
                    return (
                      <tr key={c.code}>
                        ...
                        <td>{att == null ? '' : (ok ? <span className="tag ok">met</span> : <span className="tag no">below</span>)}</td>
                      </tr>
                    );
            ...
            <div className="card comment" style={{ marginTop: 16 }}>
              ...
              <textarea id="bcomment" placeholder="Observations, areas to improve, teaching decisions…" defaultValue={b.comment} />
            ...
            <div className="card" style={{ marginTop: 16 }}>
              ...
              <table>
                <thead><tr><th>Matric</th><th>Name</th><th style={{ textAlign: 'right' }}>Average</th><th style={{ textAlign: 'right' }}>POs met</th><th /></tr></thead>
                <tbody>
                  {b.students.length ? b.students.map(id => {
                    ...
                    const ov = avgOf(po);
                    return (
                      <tr key={id} onClick={() => { dispatch({ type: 'SET_LECT', patch: { studentId: id } }); go('lect-student'); }} style={{ cursor: 'pointer' }}>
