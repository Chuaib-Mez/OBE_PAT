      ...
      <div className="wrap">
        <div className="page-head">
            ...
            {role === 'admin' ? (
              ...
              >
                {Object.values(students).map(s => (
                  <option key={s.id} value={s.id}>{s.name} · {s.matric} · {batches[s.batch].name}</option>
                ))}
              </select>
            ) : (
          ...
          <div className="stepper" role="group" aria-label="Semester">
            ...
            <span className="val mono">{sem === 'all' ? 'All semesters' : `Up to semester ${sem}`}</span>
          ...
          <div>
            <div className="card" style={{ marginBottom: 16 }}>
              ...
              <div className="toggles">
                <label className="tog">
                  <input type="checkbox" checked={student.showMine} onChange={() => dispatch({ type: 'SET_STUDENT', patch: { showMine: !student.showMine } })} />
                  <span className="swatch" style={{ background: 'var(--teal)' }} />My scores
                </label>
                <label className="tog">
                  <input type="checkbox" checked={student.showAvg} onChange={() => dispatch({ type: 'SET_STUDENT', patch: { showAvg: !student.showAvg } })} />
            ...
            <div className="card">
              ...
              <div style={{ display: 'flex', gap: 22, alignItems: 'baseline' }}>
                ...
                <div>
                  <div className="mono" style={{ fontSize: 30, fontWeight: 600 }}>{mine.filter(v => v >= PASS).length}/11</div>
              ...
              {POS.map((po, i) => {
                ...
                return (
                  <tr key={po}>
                    ...
                    <td>{ok ? <span className="tag ok">met</span> : <span className="tag no">below threshold</span>}</td>
                  </tr>
                );
              })}
            </tbody>
